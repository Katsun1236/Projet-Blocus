...
document.addEventListener('DOMContentLoaded', function() {
    setVolume(50); // Régler le volume à 50%
    ...
    returnButton.onclick = function() {
        window.location.href = 'introduction_evenementiel.html'; // Remplacer par l'URL de la page de destination
    };
...
function loadQuestion() {
    ...
    questionContainer.innerHTML = `
        <div class="qcm-question">
            ...
            `).join('')}
            <p class="question-type">${currentQuestion.correct.length > 1 ? 'Question à réponses multiples' : 'Question à réponse unique'}</p>
        </div>
...
function nextQuestion() {
    ...
    if (currentQuestionIndex < questions.length) {
        ...
        displayScore();
        document.querySelector('.next-button').style.display = 'none'; // Masquer le bouton "Question Suivante"
...
// Ajoutez cette fonction pour signaler une erreur
function reportError() {
    const questionContainer = document.getElementById('question-container');
    const questionText = questionContainer.textContent || questionContainer.innerText;
    const userEmail = 'utilisateur@example.com'; // Adresse e-mail de l'utilisateur (à remplacer par l'adresse de l'utilisateur connecté)

    // Simuler l'envoi d'un e-mail pour signaler l'erreur
    console.log(`Signaler une erreur pour la question: ${questionText} de l'utilisateur: ${userEmail}`);
